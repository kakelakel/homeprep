...
def create_task(data: dict[str, Any]) -> dict[str, Any]:
    ...
    recurrence_type = str(
        data.get("recurrence_type", "months")
    )
    if recurrence_type not in RECURRENCE_TYPES:
        raise ValueError(
            f"Unsupported recurrence type: {recurrence_type}"
        )
    ...
    reschedule_mode = str(
        data.get("reschedule_mode", "completion")
    )
    if reschedule_mode not in RESCHEDULE_MODES:
        raise ValueError(
            f"Unsupported reschedule mode: {reschedule_mode}"
        )

    task_kind = str(
        data.get("task_kind", "general")
    )
    ...
    return {
        ...
        "reminder_before_days": max(
            0,
            int(data.get("reminder_before_days", 0)),
        ),
        "enabled": bool(data.get("enabled", True)),
        "notes": data.get("notes"),
        "completion_count": max(
            0,
            int(data.get("completion_count", 0)),
        ),
...
def normalize_task(data: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized = create_task(data)

    normalized["id"] = str(
        data.get("id") or normalized["id"]
    )
    ...
    normalized["revision"] = max(
        1,
        int(data.get("revision", 1)),
    )
    normalized["completion_count"] = max(
        0,
        int(data.get("completion_count", 0)),
    )
    normalized["last_completed_at"] = data.get(
        "last_completed_at"
    )
