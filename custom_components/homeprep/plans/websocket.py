...
@websocket_api.websocket_command({vol.Required("type"): "homeprep/plan/add_item", vol.Required("plan_id"): str, vol.Required("label"): str, vol.Optional("description"): str})
@websocket_api.async_response
async def websocket_plan_add_item(hass, connection, msg):
    try:
        result = await _service(hass).async_add_check_item(msg["plan_id"], msg["label"], msg.get("description"))
    except (KeyError, ValueError):
        connection.send_error(msg["id"], "invalid_item", "Plan not found or checklist item is invalid"); return
    connection.send_result(msg["id"], result)
