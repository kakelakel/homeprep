...
class HomePrepNotificationService:
    ...
    async def async_check(self) -> None:
        ...
        for item in self._inventory_service.items:
            ...
            if days < 0 and events.get("inventory_expired"):
                pending.append((
                    f"inventory_expired:{item_id}:{expiry_raw}",
                    "HomePrep · Inventory expired",
                    f"{item_name} expired on {expiry_raw}.",
                ))
            elif 0 <= days <= expiry_warning_days and events.get("inventory_expiring"):
                when = "today" if days == 0 else f"in {days} day{'s' if days != 1 else ''}"
                pending.append((
                    f"inventory_expiring:{item_id}:{expiry_raw}",
                    "HomePrep · Inventory expiring soon",
                    f"{item_name} expires {when} ({expiry_raw}).",
                ))
        ...
        for task in self._task_service.tasks:
            ...
            if days < 0 and events.get("task_overdue"):
                pending.append((
                    f"task_overdue:{task_id}:{due_raw}",
                    "HomePrep · Task overdue",
                    f"{task_name} was due on {due_raw}.",
                ))
            elif days == 0 and events.get("task_due"):
                pending.append((
                    f"task_due:{task_id}:{due_raw}",
                    "HomePrep · Task due today",
                    f"{task_name} is due today.",
                ))
            elif 0 < days <= reminder_days and events.get("task_reminder"):
                pending.append((
                    f"task_reminder:{task_id}:{due_raw}",
                    "HomePrep · Task due soon",
                    f"{task_name} is due in {days} day{'s' if days != 1 else ''} ({due_raw}).",
                ))
    ...
    async def _async_send(self, target: str, title: str, message: str) -> bool:
        ...
        try:
            ...
            return True
        except Exception:  # Home Assistant service errors should not stop other recipients.
            return False
    ...
    def _normalize_settings(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        events = {
            key: bool(raw_events.get(key, default))
            for key, default in DEFAULT_SETTINGS["events"].items()
        }
